...
typedef bool Period[PERIOD_CNT];
...
{
    string course;
    bool is_chosen[CLASS_CNT];
};
struct Teacher
{
    string name;
    int free_days_cnt;
    Period have_time[DAY_CNT];
...
Teacher initialize_teacher(string input_line)
{
    ...
    // initialize_teacher_day(teacher);
    input >> course_cnt;
...
bool is_teacher_valid(Course course, Teacher teacher, Index index)
{
    ...
    for (Tutor course_name : teacher.courses)
...
Class plan(Plan &cls, vector<Teacher> &teacherList, vector<Course> &courseList, Index index)
{
    ...
    // if (fit_course.teacher_name != EMPTY)
...
void print_teacher(string teacher_name, int period){
    cout << teacher_name << ": " << 
    PERIOD[period].first << ' ' << PERIOD[period].second << endl;
}
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            for (int j = 0; j < 5; j++)
                for (int k = 0; k < 3; k++)
                    ...
                    {
                        is_found = true;
                        print_teacher(cls[i][j][k].teacher_name, k);
                    }
            if (is_found == false)
                cout << "Not Found\n";
        }
