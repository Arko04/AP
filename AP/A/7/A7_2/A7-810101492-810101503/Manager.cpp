...
void Manager::handle_league_standing_command()
{
    ...
    {
        LeagueInfo league_info(real_teams[i]->get_league_info());
...
bool Manager::is_valid_for_set_captain(const vector<string> &words)
{
    if (words.size() < EXPECTED_ARGUMENTS_NO_FOR_SET_CAPTAIN)
        return false;
...
void Manager::process_for_set_captian(const vector<string> &words)
{
    if (!is_valid_for_set_captain(words))
        throw BadRequest();

    string player_name = get_player_name(words);
    active_user->handle_set_captain(player_name);
...
void Manager::process_player_command(const vector<string> &words)
{
    ...
    if (words.size() == MIN_EXPECTED_ARGUMENTS_NO_FOR_PLAYERS || (words.size() == 6 && words[5] == RANKS))
        real_team->print_players(words.size() == 6);
    else
    {
        string status = words[5];
        real_team->print_players(words.size() == MAX_EXPECTED_ARGUMENTS_NO_FOR_PLAYERS, status);
    }
}
...
void Manager::print_users_ranking(const vector<pair<string, double>> &fantasy_teams)
{
    for (int i = 0; i < fantasy_teams.size(); i++)
    {
        cout << i + 1 << NUMBER_DELIMITER << TEAM_NAME << CHAR_DELIMITER << fantasy_teams[i].first << DATA_DELIMITER
             << POINT << CHAR_DELIMITER << fixed << setprecision(1) << floor(10 * fantasy_teams[i].second) / 10 << endl;
    }
}
...
void Manager::handle_user_rankings()
{
    ...
    if (fantasy_teams.empty())
        throw Empty();
    sort(fantasy_teams.begin(), fantasy_teams.end(), Utils::comp_score);
...
bool Manager::is_valid_for_show_budget()
{
    if (logged_admin == true)
        throw PermissionDenied();
    else if (logged_user == false)
...
void Manager::process_for_show_budget(const vector<string> &words)
{
    if (!is_valid_for_show_budget())
        throw BadRequest();
    active_user->show_budget();
}
