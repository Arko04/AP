...
void Manager::assign_mission(vector<string> command_fields)
{
    ...
    auto mission_copy = mission->clone();
    // shared_ptr<Mission> mission1 = shared_ptr<Mission> (*mission);
    //mission->set_mission(find_mission(mission_id));
    shared_ptr<Driver> driver = find_driver(driver_id);
    // cout <<"l\nl\nl\nl\n";
    driver->insert_mission(mission_copy);
}
...
//class SubMission : public TimeMission , CountMission , DistanceMission{};